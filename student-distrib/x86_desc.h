...
typedef union idt_desc_t {
    ...
    struct {
        ...
        uint8_t  reserved4;
        uint32_t reserved3 : 1;  // selector for interrupt/trap gate. 1 -> Trap. 0 -> Interrupt
        uint32_t reserved2 : 1;  // hard coded to 1 for our purposes
        uint32_t reserved1 : 1;
        uint32_t size      : 1;  // gate size=1 -> 32-bit and size=0 -> 16-bit
        uint32_t reserved0 : 1;
        uint32_t dpl       : 2;  // selector for privledge level. 0 -> only kernel level code can access. 3 -> user level code can access.
        uint32_t present   : 1;  // mark wether IDT entry is present or not
        uint16_t offset_31_16;
    } __attribute__ ((packed));
} idt_desc_t;

/* An Segment Selector entry (goes into IDT Entries) */
typedef union seg_selector_t {
    uint16_t val;
    struct {
        uint32_t rpl    : 2;
        uint32_t ti     : 1;
